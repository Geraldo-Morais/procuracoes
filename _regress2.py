...
HTML = pathlib.Path(__file__).resolve().parent / "index2.html"
if not HTML.is_file():
...
with sync_playwright() as p:
    ...
    # busca por nome e por CPF, e etiqueta do beneficio
    pg.evaluate("abrirHistModal()"); pg.wait_for_timeout(300)
    tags = pg.evaluate("()=>Array.from(document.querySelectorAll('#histLista .hist-tag')).map(e=>e.textContent)")
    check(len(tags) == 2 and all(t for t in tags), f"historico: etiqueta do beneficio em cada linha (got {tags})")
    pg.fill("#histFiltro", "maria"); pg.wait_for_timeout(300)
    r1 = pg.evaluate("()=>Array.from(document.querySelectorAll('#histLista .hist-nome')).map(e=>e.textContent)")
    check(r1 == ["MARIA APARECIDA DE SOUZA"], f"historico: busca por nome (got {r1})")
    pg.fill("#histFiltro", "111444"); pg.wait_for_timeout(300)
    r2 = pg.evaluate("()=>Array.from(document.querySelectorAll('#histLista .hist-nome')).map(e=>e.textContent)")
    check(r2 == ["JOAO CARLOS PEREIRA"], f"historico: busca por CPF, so digitos (got {r2})")
    pg.fill("#histFiltro", "zzzz"); pg.wait_for_timeout(300)
    check("Nada encontrado" in pg.inner_text("#histLista"), "historico: busca sem resultado avisa")
    pg.fill("#histFiltro", ""); pg.wait_for_timeout(250)
